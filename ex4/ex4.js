...
(function() {
  ...
  var Iterator = (function() { //get's executed, we create a closure & now we have access to private var's inside
		'use strict';
    var _idx, __obj__;
  ...
  AddressList.toString = function() {
    ...
    if (this.entryCount > 0) {
      ...
      while ((addr = this.next()));
			// could have looped over the adress array b/c it's a private data member
			// but that's not object oriented. The OO approach says, if I'm an interator,
			// I'm going to iterate over myself
  ...
  AddressBookEntry.toString = function() {
    ...
    ret += this.name;
    if (this.addresses.entryCount > 0) {
      ret += '\n' + this.addresses;
    }
    return ret;
  };
  ...
  AddressBook.init = function(aBook) {
    ...
    if (aBook && Object.isPrototypeOf.call(AddressBook, aBook)) {
      ...
      while ((entry = aBook.next()));  //Object oriented iteration
    }
  };
  ...
  AddressBook.next = function() {
    ...
    if (_idx < (this.entries.length - 1)) {
      _idx = this.__iter__.next();  //Object oriented iteration
      return this.entries[_idx];
    }
  };
  ...
  AddressBook.toString = function() {
    var ret = '',
      entry;
    if (this.entryCount > 0) {
      this.resetIterator();  //Object oriented iteration
      entry = this.current();
      do {
        ret += (ret !== '' ? '\n---------------\n' : '') + entry;
      }
      while ((entry = this.next())); //Object oriented iteration
    }
    return ret;
  };
  ...
  while ((entry = mybook.next())); //Object oriented iteration
  ...
  while ((entry = yourbook.next())); //Object oriented iteration
